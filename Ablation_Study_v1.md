...
> **⚠️ 2026-07-22 数据待更新**：本文档下面全部结果都是在 `_switch_score()` 有个方向反了的 bug
> （防御风险算成了进攻收益，详见 `Experiment_Log.md` 同日期条目）修复**之前**跑出来的，描述的不是
> 当前这版智能体的真实行为。已经在重新跑一遍完整消融（7 组配置 × 3 次重跑），跑完会重写下面的
> "多次重跑确认结果"一节。旧数据先保留作为"修复前"的历史对照，不要直接拿来当结论用。
