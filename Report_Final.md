...
One instance of "unreliable measurement tools" is worth naming concretely: `max_damage-uber` looked like a stable loss across seven or eight unrelated code changes — the kind of consistency that should count as strong evidence. It wasn't the decision logic, though; our own ablation script was quietly leaking connections across repeats, and that fully explained the pattern. Rerunning a test rules out bad luck, but not a measuring tool that's broken in a consistent way [9] — the two produce data that looks identical.
...
### 4.2 Where the Design Held Up, and Where It Didn't

Looking at the finished system as a whole rather than any single piece, the clearest strength is that the fit between playstyle and algorithm actually held under evaluation. §3.2 shows the decision logic, not the roster, is what turns a 1/15 result into 15/15; §3.3 shows the two pieces we deliberately hard-scripted instead of leaving to greedy scoring — the hard-KO short-circuit and the opening script — are exactly the ones that cost real games when removed, meaning the call to script them instead of scoring them was the right one. `self.decision_log` also delivered on what §2's Explanation Facility framing promised: every result discussed in §3 traces back to a specific, inspectable decision rather than a black box.

The clearest gap sits in team selection, not battle logic. §2.1 established that `teampreview_opponent_team` reveals the opponent's full roster before we commit to a lead — information the design only partly acts on: a counter table is built from it and used by in-battle scoring, but `teampreview` still always returns `"/team 1"`, so an opponent whose revealed roster specifically counters our fixed lead gets no adaptive response at the one point that would matter most. That's a deliberate scope cut, not an oversight — teampreview's uniform randomness (§1) made in-battle reaction the higher-value place to spend the time available — but it means real, available information is left partly unused.
