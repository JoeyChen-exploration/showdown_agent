...
This is a structural limit of a rule-based expert system operating in a non-deterministic environment: the rules themselves can be fully deterministic and traceable, but the environment they run in is not, so correctness can only ever be argued statistically, never proven from one run. That statistical argument has its own blind spot, too — reproducibility across reruns only rules out random noise, not a *biased but deterministic* measurement tool, which can produce data indistinguishable from a real, stable effect [9]. We hit this directly: `max_damage-uber` looked like a stable loss across roughly seven or eight unrelated code changes, which we initially took as strong evidence of a structural limitation in the decision logic — until it turned out to be our own ablation script leaking connections across repeats, not a property of the system under test at all.
