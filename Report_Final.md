...
### 4.2 Limitations: What the Analysis Didn't Cover

§2.1 established that `teampreview_opponent_team` reveals the opponent's full roster before we commit to a lead — real information the design only partly acts on. A counter table is built from it and used by in-battle scoring, but `teampreview()` still always returns `"/team 1"`; team order itself never adapts to what's revealed, so an opponent whose roster specifically counters our fixed lead gets no adaptive response at the one point that would matter most. That's a deliberate scope cut, not an oversight — teampreview's uniform randomness (§1) made in-battle reaction the higher-value place to spend the time available — but real, available information is still left partly unused.

The deeper limitation is one every rule-based expert system shares, not something specific to this codebase: the knowledge base only knows what §2.2's analysis actually found. Every threat rule traces back to a pattern that recurred across the 15 bots we manually inspected — paralysis, priority, Choice locks, hazards, Knock Off. A mechanic or interaction that simply never showed up in that analysis has no rule at all, and no fallback beyond the general scoring formula's raw numbers; the system can't notice a category of threat it was never told to look for, the way a learned model might still respond, however imperfectly, to something outside its training distribution. Ablation (§3.3) can tell us our encoded rules are working; it can't tell us what we failed to encode in the first place.
