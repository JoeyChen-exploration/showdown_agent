...
- `analysis/`（仓库根目录，2026-07-21 新增）是我们自己的消融实验/复盘工具，**不属于提交范围**
  ...
  会热加载 `wche652.py`、按配置覆写它顶层的可调常量（`ENABLE_HARD_KO`/`SWITCH_COST` 等），支持
  `--repeats N` 多次重跑取均值/标准差，跑完整个 15-bot 梯队后把结果、`decision_log`、
  `battle.observations` 落盘到 `analysis/results/`。`analysis/plot_ablation.py`（2026-07-22 新增，
  依赖 `matplotlib`，已装进 `pokemon/` venv，不影响提交用的 `players/requirements.txt`）读
  `analysis/results/summary.csv` 画消融结果对比图，输出到 `analysis/figures/`，供 `Report_Draft.md`
  引用。`analysis/trace_matchup.py`（2026-07-22 新增）针对单个 (配置, 对手) 组合单独跑一次 Bo3，
  输出写到 `analysis/results/_trace/`（跟正式消融数据目录分开，不会覆盖已跑好的多次重跑聚合结果），
  用于复盘具体某个对手为什么赢/输，不用为此重跑全部 15 个 bot。
...
- **判断"某属性打某属性是超克/被抗/免疫"之前先查 [Type_Chart.md](Type_Chart.md)，不要凭印象/经验猜**——
  之前好几次判断错误（以为只有毒系能打 Arceus-Fairy，其实钢系也超克；Judgment 类型没跟着阵型
  动态解析）都是这么来的，本可以提前避免。
...
- `_choose_move` 的策略设计（决策架构、打分公式、各精灵的战术角色）记在 [bots_strategy.md](bots_strategy.md)，
...
- Report（占分 40%，见 [AssessmentDescription.md](AssessmentDescription.md)）要求 Design/Evaluation/
  ...
  以后每次更新 `Experiment_Log.md`/`bots_strategy.md`，顺手看一下 `Report_Outline.md` 里的素材缺口
