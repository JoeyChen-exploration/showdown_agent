...
## v1 → v2 分界线（2026-07-23）

v1（六处正确性修复 + 消融确认默认参数最优）到此告一段落。往下的条目是 v2：不再是"把现有逻辑
修对"，而是给决策框架加新能力（如太晶化）或解决结构性缺口（如 `max_damage-uber` 超时问题）。
Report 相关工作（Design/Evaluation/Reflections）暂缓，先集中精力推进 v2、多积累数据和实验记录，
留到最后一起写报告。
