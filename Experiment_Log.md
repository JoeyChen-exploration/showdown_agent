...
## 2026-07-24 v4 起步：`_we_move_first` 补上招式先制（issue #15）

issue #7 的两处修复收尾后，正式开始 v4。用户从几个候选方向里选了"补全招式先制（`move.priority`）"。

**问题**：`_we_move_first`（判断"这回合我方先手还是对面先手"的函数）之前只比双方速度，完全没看
先制等级——真实机制里先制等级会直接压过速度（比如 +2 优先度的招式，不管谁速度快都会先出手）。
这个判断结果会喂给 `_survivability_factor`（判断这回合铺垫/回血安不安全）和 `_log_decision`
记录的 `we_move_first` 字段，所以对面一个先制招式理论上能让"这回合安全"这个判断算错。

**先查清楚事实，不凭印象设计**：

1. 查 `gen9moves.json`（poke_env 自带的招式数据）确认我方全队 22 个招式（6 只精灵）优先度
   全部是 0——没有一个先制招式。这意味着这个修复**只需要看对面一侧**，我方自己的优先度永远
   不会反过来影响这个判断，不需要做"双方都比一下候选招式优先度"这种更复杂的对称设计。
2. 查全部 5 档对手队伍文件，确认真实存在先制招式，不是假设的边界情况：极速拳 Extreme Speed
   （+2，NU/OU 档）、真气拳 Bullet Punch（+1，UU 档）、突袭 Sucker Punch（+1，Uber 档）。
3. 中途讨论过"要不要对没见过的招式也做预判（比如对面属性克制我方就假设它可能带先制）"——
   否掉了这个思路，理由两条：先制招式是招式/精灵本身的属性，跟属性克制没关系，用克制关系去
   预判先制经常会猜错；而且如果对"没见过的招式"一律往"可能是先制"那边偏，铺垫/回血招式又会
   变成几乎永远"不安全"，跟今天早些时候刚修好的 `_survivability_factor` 门槛太严是同一种问题，
   等于白修。

**修复**：只对**已经亮过相**的对面先制招式反应——跟现有 `_incoming_threat_score`"已知招式用
已知，没见过就用保守默认值"是同一套逻辑，不是另外发明的新规则：

```python
if any(m.priority > 0 for m in opp.moves.values()):
    return False
```

放在原来的速度比较之前，一旦命中直接判定"对面先手"，不再看速度。

**明确不做的**（保持范围小、可解释，不做成一个大而全的先制系统）：
- 王者盾牌/守住这类 +3/+4 优先度的守护类招式——不造成伤害，不会带来"以为安全结果没安全"这种
  威胁，是另一个问题（预判/应对守住），不在这次范围内。
- 吼叫这类 -6 优先度的强制换人招式——这类招式是**最后**出手，不是先手，不会触发这次要修的
  那种失败模式。
- 突袭（Sucker Punch）真实的附加条件（只有目标这回合选的是攻击招式才会命中，如果目标用的是
  变化招式，突袭会直接落空）没有建模——一旦看到过这个招式就当成无条件先制处理，多数情况
  （我方在攻击）这个假设是保守、安全的，只有"我方这回合恰好也选了铺垫招式"这一种特定情况下
  会偏保守（实际上突袭会落空，但我们会当成命中来判断），复杂度换不来足够的收益，先不做。

已发 code review，等结果出来跑消融验证（重点看有没有回归，以及能不能在对面出过先制招式的
那几局里实际看到 `we_move_first` 被正确判成 `False`）。

## 2026-07-24 先制修复验证：逻辑本身没问题，但代价没查清楚具体原因，最终决定保留

**code review 结果**：干净通过，没有 Critical/Warning。核实了 `opp.moves` 确实只包含"这场对局里
已经用过的招式"（不是完整招式池），`move.priority` 对 `opp.moves` 里任何可能出现的招式对象都
安全取值，两处调用方（`_survivability_factor`/`_log_decision`）语义都合理。给了一条 Suggestion：
补充说明"没检查这个先制招式当下能不能用（锁招/PP 耗尽）"这个简化点——已经采纳，加了对应注释，
说明这个简化的方向跟这份文件一贯的风险取向一致（宁可高估对面威胁，也不要低估）。

**直接单测确认逻辑本身没问题**：用真实 poke_env 对象构造"对面亮过极速拳"这个场景，直接调用
`_we_move_first` 验证：

```
opp 亮过 Extreme Speed（+2 优先度）→ we_move_first(科依大 vs 帝牙卢卡) = False（正确，
尽管我方明显更快，先制招式应该压过速度）
opp 没有任何先制招式 → we_move_first(科依大 vs 呆呆兽) = True（正确，正常按速度判断）
```

**消融验证发现一个真实的、可复现的代价，但没查清楚具体因果**：跑了两次独立 `--repeats 3`
消融确认，`baseline` 从干净的 15.00/15 都降到了 14.67/15（14/15, 14/15, 15/15 这类分布），
翻 `per_bot_summary.csv` 两次都是同一个对手——`simple-uber` 从稳定 3/3 掉到 2/3。

为了确认这不是巧合，做了一次 A/B 对照：临时把这条先制判断禁掉（`if False and ...`），用
`trace_matchup.py` 单独对 `simple-uber` 连测 3 次，**3 次全部 winrate=1.00**——加上这个改动
之前 issue #7 收尾时提交的那次基准（同样是 3/3 干净），一共 6 次"没有这条修复"的样本全部干净，
跟"有这条修复"的两次消融（各自内部 3 次重跑，一共 6 个 repeat 样本）都稳定复现同一个对手掉分，
方向一致、可复现——不是单次运气的假信号。

但翻输的那局详细日志想找具体原因时，没找到——对面真正秒掉我方的那一下用的是白刃斩（Behemoth
Blade，0 优先度普通招式），不是先制招式；这局里对面唯一带先制招式的关刀（Sucker Punch）自始至终
没被用过。也就是说不是"先制招式直接触发误判"这种直接因果，更像是这条改动让某几个回合"谁先手"
的判断变了之后，通过战斗本身的随机性（对手随机首发出场顺序、命中/暴击判定）间接影响了整局走向。
对局本身没法固定随机种子重放，做不到"完全相同的一局、只改一行代码"这种严格对照实验，所以这个
因果链条没能完全查清楚，只能确认"改了之后这个代价稳定出现"这个相关性。

**决定：保留这个修复**。理由（用户明确判断）：这套系统能真正尝试、验证的机制基数本来就有限，
多一个确认正确的通用机制（不是针对某个对手的补丁，逻辑本身单测验证过完全符合 Gen9 真实规则）
本身是有价值的，这点集中在一个对手身上、幅度不大的代价可以接受——跟之前 `SETUP_BOOST_WEIGHT`
"要不要为了拿到真正能用的能力承担一点点代价"是同一类判断，结论也是同一个方向（要）。

**下一步**：走完流程——合并分支、关 issue #15、更新 `bots_strategy.md` 相关引用。

## 2026-07-27 issue #16 后续调查：能不能用固定随机种子重放来查清楚因果，结论是查了但做不出来

**目标**：上一条记录留了一个尾巴——"改动带来的代价方向确定、但具体因果链条没查清楚"，原因是
"对局本身没法固定随机种子重放"这句话当时是直觉判断，没有真正去查过底层是不是真的做不到。这次
专门花时间核实这句话到底站不站得住脚，全程只读代码、不改 `wche652.py`，也不改仓库里任何跑分
相关的文件。

**核实过程（都是读源码，不是猜的）**：

1. `pokemon-showdown` 本地服务器（`sim/prng.ts`、`sim/battle.ts`）确实支持指定种子——`Battle`
   构造函数接受 `options.seed: PRNGSeed`，`RoomBattleOptions`（`server/room-battle.ts:490`）也有
   同名字段，说明底层机制本身是存在的，不是完全没有这个能力。
2. 但把整个 `server/` 目录搜了一遍"哪里会真正给 `options.seed` 赋值"，除了类型定义本身和
   `room-battle.ts:575` 那一处透传赋值之外，**没有任何调用点会从客户端请求（challenge/matchmaking
   等正常对战创建流程）里读出种子填进去**——也就是说，通过 `poke_env` 实际使用的这条标准协议
   路径，种子永远是 `PRNG.generateSeed()` 随机生成的，没有开放给客户端指定。
3. 反过来查 `poke_env` 自己的源码（`pip` 装在 `pokemon/lib/python3.12/site-packages/poke_env`），
   全库搜"seed"，命中的全是招式名字"Leech Seed"和 Gymnasium 强化学习环境包装器
   （`environment/env.py`/`single_agent_wrapper.py`）自己的 `seed()` 方法，跟对局本身的战斗
   随机数完全无关——`poke_env` 的 Player API 层面根本没有暴露任何跟"控制这局要用哪个随机种子"
   相关的参数。

**结论：这条路在我们用的这套技术栈里确实走不通，不是猜测，是查完源码之后的确定结论。**

**更进一步的结构性问题：就算种子暴露出来了，也不能完全解决这次要查的问题**。Pokemon 对战的
随机数是按"游戏里实际发生的事件顺序"依次消耗的一条流——本回合谁先手、放不放先制这类判断一旦
在某一回合分叉（这正是我们要测的东西：加了先制判断 vs 不加），从那一回合起双方发生的具体事件
就不一样了，后续每一次命中判定/暴击判定/异常状态判定消耗随机数的顺序也就跟着不一样。也就是说
"同一个种子，从头重放，只改一回合的决策"得到的不是"一局完全相同、只有一个变量不同"的严格对照，
而是"从那一回合起，两条时间线本来就应该长得不一样"——这不是需要排除的干扰，是决策本身真实带来
的后果。要做出真正干净的对照实验，需要能在战斗中途某个具体回合"存档"（同时保存 Python 侧
`wche652.py`/对手 `SimpleHeuristicsPlayer` 的内部状态和 Node 侧战斗引擎的完整状态+随机数流
位置），分叉出"这回合选 A"和"这回合选 B"两条分支，各自独立多次重跑，比较两条分支的平均结果——
这类似蒙特卡洛树搜索里"从固定节点展开多次模拟"的做法，但我们的对局双方决策逻辑是 Python
（`wche652.py` + poke_env 内置的 `SimpleHeuristicsPlayer`），战斗引擎是独立的 Node 进程，
中途存档/回放需要跨语言桥接两边的完整状态——这是一个独立的、体量不小的工程项目，不是给
`_we_move_first` 这一行代码的验证任务应该背负的成本，跟这条修复本身"代价小、逻辑已验证正确"
的现状不成比例，这次不投入去做。

**退而求其次：给出目前证据能支撑的最佳猜想，而不是继续猜"因果链条"**。已知两条线索：
1. 先制判断本身有一个已知、写进代码注释的简化点——不检查这个先制招式当下是否真的能打得出来
   （Choice 系列锁招后被锁进另一个招式、或者 PP 已经打空），只要这局里出现过就当成"一直存在
   的威胁"。`simple-uber` 队伍里带 Sucker Punch 的关刀正是这类会被 Choice 道具锁招的候选——
   一旦关刀被锁进别的招式，先制判断依然会保守地当它随时可能先制，可能导致我方在关刀其实打不出
   先制的回合，不必要地放弃铺垫/换成更保守的选择。
2. 铺垫招式（`_survivability_factor` 把关的那类）的价值是延后兑现的——某一回合"因为怕先制而
   没铺垫"这个选择，损失不会立刻体现在那一回合的血量变化上，而是体现在之后几回合本该有的强化
   没叠上。这跟实际翻查的那局输局现象吻合：真正把我方打死的是一次纯速度压制（白刃斩，不是先制
   招式），先制招式本身全程没被用过——如果这局输的根本原因是"早几回合本该铺垫却没铺垫，导致
   后面速度/输出没压过对面"，先制招式"看似没参与那一下"和"确实是这条修复间接导致的"是可以同时
   成立的，不矛盾。

这个猜想目前**没有、也没法在合理成本内被完全证实**——记在这里，同时也是 Reflections 里"专家系统
在非确定性环境下，有些相关性只能停在相关性、没法被追到完全确定的因果"这条认识论局限的一个具体、
真实核实过（不是编的）的案例。
