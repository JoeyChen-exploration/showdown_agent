...
找出哪些参数值得深挖，不直接下结论。第二轮是多次重跑确认——对全部 9 组配置各重跑 3 次，取均值和
标准差，把噪声和真实效果分开；决策逻辑本身也经过几轮修正（包括几处影响面较大的正确性修复，以及
一次消融工具本身的修正——详见 4.4 节），下面呈现的是当前最新数据。完整数据见 `analysis/results/`，
逐条解读见 [Ablation_Study.md](Ablation_Study.md)。
...
1. **`baseline`（全部参数用默认值）打出满分**（15.00/15，3 次重跑标准差 0），是目前综合表现最好
   的配置之一，跟另外 5 个同样打满分的配置（`setup_weight_15`/`switch_defense_weight_30`/`_90`/
   `switch_cost_0`/`switch_cost_50`）并列。
...
3. **权重类参数在测过的取值范围内基本不敏感**：`SETUP_BOOST_WEIGHT`/`SWITCH_DEFENSE_WEIGHT`/
   ...
   本身已经是满分之一，暂不改动。这跟更早一版数据（`SETUP_BOOST_WEIGHT`/`SWITCH_COST` 的"最优方向
   反复反转"）不一样——4.4 节会说明这个差异本身的来源。
