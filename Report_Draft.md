...
"信号不明确"的 7 组配置各重跑 3 次，取均值和标准差，把噪声和真实效果分开；决策逻辑本身也在这个
过程中经过一轮修正，下面呈现的是修正后的最新数据。完整数据见 `analysis/results/`，逐条解读见
...
|---|---|---|---|---|
| `baseline` | 不改，v1 默认值 | 13.67 | 0.76 | 12–15 |
| `no_hard_ko` | 关闭硬短路 KO 规则 | 13.67 | 0.29 | 13–14 |
| `no_opening_script` | 关闭 Ribombee 固定开局脚本 | 13.67 | 0.29 | 13–14 |
| **`setup_weight_15`** | 铺垫打分权重 30→15 | **14.00** | **0.00** | 14–14 |
| **`setup_weight_45`** | 铺垫打分权重 30→45 | **14.00** | **0.00** | 14–14 |
| `switch_cost_0` | 换人成本 25→0 | 13.67 | 0.29 | 13–14 |
| `switch_cost_50` | 换人成本 25→50 | 13.67 | 0.29 | 13–14 |

三个观察：

1. **`SETUP_BOOST_WEIGHT` 往两个方向调（15 和 45）都稳定拿到全场最高的 14/15（标准差 0）**，
   明显高于默认值 30（`baseline` 均值 13.67）。样本目前还不够分出 15 和 45 哪个更稳，但方向上都
   比默认值好，这是这组数据里最清楚的正向信号。
2. **其余几个改动（关硬短路 KO、关开局脚本、调换人成本）目前都跟 baseline 处在同一档**
   （均值都是 13.67），没有哪个能单独拿出来当作"确认更好"的改动——跟前一版数据比这是个明确的
   修正：之前一度以为"关开局脚本"是个干净的正向发现，现在看跟不改动没有统计上的差异。
3. **`max_damage-uber` 这场依然是全表最不稳定的一格**：只有 `baseline` 偶尔能赢（胜率均值 0.33，
   标准差 0.58，全表最高），其余 6 组配置稳定输（0/3）。这个模式在数据修正前后完全一致，说明
   这场的胜负确实由是否卡进 90 秒超时决定，不受这 7 个参数影响——继续在这几个权重上找不会有
