...
这是从 [Ablation_Study_v1.md](Ablation_Study_v1.md) 同源的决策架构参考图裁剪出的精简版——只留
顶层四条分支和一句话触发条件，去掉了嵌套子判断和具体打分公式，专门为塞进 6 页篇幅限制画的；
公式细节和团队构成放回正文文字里讲。生成脚本：`analysis/plot_decision_flow.py`。）*

### 2.1 方法论声明

*（借助 pokeaimmd.com/randomizer 搭建队伍——专家系统的贡献在决策逻辑，不在队伍本身；
这一点要点清楚，避免读者误以为"选对精灵"是我们的设计工作。）*

### 2.2 调研方法

*（解析 `bots/teams/` 全部 30 只精灵的属性/特性/道具/招式分布，对应 [Opponent_Roster.md](Opponent_Roster.md)。
呼应 Introduction 六个 factors 里"系统性调研而非经验直觉"这条方法论主张。）*

### 2.3 调研发现 → 设计决策对照表

*（`Opponent_Roster.md`"跨档通用观察"一节：拍落 Knock Off 频率高 / 钉子流常见 / 麻痹类招式多 /
Zapdos-Galar 的 Defiant 特性 / Choice 系锁招道具——每条观察各自怎么反映进了具体设计决策，
建议做成一张两栏表：观察 → 设计回应。）*

### 2.4 决策架构的演变过程

*（从最初的 if-elif 优先级链，到现在的"硬短路 + 单回合价值打分"，体现理解如何演变，
对应 `1st_strategy.md` 里的讨论过程——尤其是"为什么放弃多回合搜索/minimax"这一步的推理，
呼应 2.1 图里那四层级联的来历。）*

### 2.5 具体机制设计

*（每个小节：设计动机 + 实现方式，公式细节可以从 2.1 的图里补回来）*

- **2.5.1 硬短路 KO 规则**——为什么要有这条短路（对面非博弈型 AI，能斩杀就没必要犹豫），
  以及威力盲区那个 bug 案例是否值得提一句（也可以留给 Reflections 4.2）
- **2.5.2 换人评分**——进攻端/防御端双向打分，`_switch_score` 公式，以及那个方向反了的
  bug 本身要不要写（按之前的决定，报告里只呈现数据观察和结果提升，不展开调试过程——
  这里更适合只讲"现在公式长什么样、为什么这样设计"）
- **2.5.3 铺垫招式打分**——速度优先判断"扛不扛得住"，`_survivability_factor` 的先手/后手
  不同阈值
- **2.5.4 Ribombee 固定开局脚本**——Sticky Web → Stun Spore/U-turn 分支，为什么对开局
  单独写死脚本而不是完全交给通用打分

### 2.6 明确不做的决策与理由

*（不做多回合搜索/minimax，因为对手是非博弈、非适应性的固定 AI；v1 明确不实现太晶化——
两条都要给出"为什么现在不做"的理由，不是"没空做"。）*

### 2.7 局限性讨论

*（打分权重是启发式初始值，不是理论推导出来的；对手速度估算用保守的满配估计近似真实值——
这里自然过渡到 Evaluation/Reflections 里的具体案例，不用展开细节，指过去就行。）*
...
[Ablation_Study_v1.md](Ablation_Study_v1.md)。
