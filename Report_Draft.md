...
> **⚠️ 2026-07-22 数据待更新**：3.2/3.3/3.4 节引用的消融数据是在换人打分公式的一处修正
> （详见 `Experiment_Log.md` 同日期条目、issue #6）之前跑出来的，正在重新跑一遍拿修正后的准确
> 数据，跑完会重写这几节。3.1 的方法论本身不受影响（讲的是"怎么判断一个差异是不是噪声"这件事，
> 不依赖具体数字）。写进报告的角度是数据观察和结果提升本身（修正前后的胜场对比），不展开代码
> 层面的调试过程。
