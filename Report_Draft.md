...
2. **权重类参数目前没有哪个改动能稳定超过默认值**：`SETUP_BOOST_WEIGHT`/`SWITCH_COST` 的最优
   方向在反复消融中多次反转，说明它们的最优值对系统其它部分很敏感，不是一次测完就能定死的常量；
   结合默认值现在最优这一点，暂不改动。
3. **`max_damage-uber` 依然是全表最不稳定的一格**：只有 `baseline` 偶尔能赢（胜率均值 0.33，
   标准差 0.58，全表最高），其余 6 组配置稳定输（0/3）。这个模式连续七轮独立重跑（跨越正确性
   修复和后续加的新能力，包括 v2 专门针对这个对手设计的两个功能）完全没变过，是目前确认过的
   结论里证据最充分的一条：这场的胜负纯粹由是否卡进 90 秒超时决定，不是当前决策范式（单回合
   价值打分，不做多回合规划）能覆盖的问题——*（这个结论背后"为什么单回合贪心打分结构性地处理不了
   这类问题"的具体推理，是很好的 Reflections 素材，待写）*。
