...
7 组配置各重跑 3 次，取均值和标准差，把噪声和真实效果分开；决策逻辑本身也经过几轮修正，下面呈现
的是当前最新数据。完整数据见 `analysis/results/`，逐条解读见 [Ablation_Study_v1.md](Ablation_Study_v1.md)。

![Ablation: mean bots beaten per config, 3 repeats each](analysis/figures/ablation_mean_beaten.png)

| 配置 | 改动 | mean 胜场 / 15 | stdev |
|---|---|---|---|
| `baseline` | 不改，v1 默认值 | 14.00 | 0.50 |
| `no_hard_ko` | 关闭硬短路 KO 规则 | 14.00 | 0.00 |
| `no_opening_script` | 关闭 Ribombee 固定开局脚本 | 13.00 | 0.00 |
| **`setup_weight_15`** | 铺垫打分权重 30→15 | **14.00** | **0.00** |
| `setup_weight_45` | 铺垫打分权重 30→45 | 13.67 | 0.29 |
| `switch_cost_0` | 换人成本 25→0 | 13.67 | 0.29 |
| `switch_cost_50` | 换人成本 25→50 | 14.00 | 0.00 |

三个观察：

1. **`SETUP_BOOST_WEIGHT=15` 是目前唯一一个在两轮独立重跑里都确认更好的具体改动**，稳定拿到
   全场最高的 14/15（标准差 0），明显高于默认值 30。上调到 45 则跌到中间档（13.67），不再有
   正面证据——只有下调这个方向站得住。
2. **多数配置（4/7）并列拿到全场最高的 14/15**（`baseline`/`no_hard_ko`/`setup_weight_15`/
   `switch_cost_50`），整体胜场比早期数据进一步上移。`no_opening_script` 反而是这轮里唯一
   明显更差的一组（13/15），说明这几个开关/权重的"最优值"会随系统其它部分的改动而变化，不是
   一次测完就能定死的常量。
